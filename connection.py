...
- Lors de la saisie du code reçu par Telegram, ajoutez 'aa' devant.
  → Ex : code reçu 12345 → entrer : aa12345
        ...
        # Create a unique session name for this phone number
        session_name = f"session_{user_id}_{phone_number}"
        
        # Clean up any existing session files for this user/phone to avoid conflicts
        import glob
        existing_sessions = glob.glob(f"session_{user_id}_{phone_number}.*")
        ...
        try:
            # Create new client for the phone number with fresh session
            new_client = TelegramClient(
                session_name,
                int(os.getenv("API_ID")),
                os.getenv("API_HASH"),
                device_model="TeleFeed Bot",
                system_version="1.0",
                app_version="1.0"
            )
            ...
            # Ensure we're not logged in already to avoid IP conflicts
